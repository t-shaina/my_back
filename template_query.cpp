...
QStringList Template_query::decoding_json_object(const QJsonValue& object){
        ...
        for (int i = 0; i < object_array.size(); i++){
            qDebug() << "in decoding json object element is" << object_array.at(i).toString();
...
QJsonObject Template_query::select_all_for_record(int film_id, const QString parent_type_connection){
 ...
 bool Template_query::exist_query(const QJsonObject& object, const QString parent_type_connection){
...
Json_creator  Delete_query::process_request( QJsonObject &object){
        ...
        return Json_creator(request_code, !deletion_state, email, array);
}
